...
const char *faceCascadeFilename = "../data/lbpcascade_frontalface.xml";     // LBP face detector.
...
const char *eyeCascadeFilename1 = "../data/haarcascade_eye.xml";               // Basic eye detector for open eyes only.
const char *eyeCascadeFilename2 = "../data/haarcascade_eye_tree_eyeglasses.xml"; // Basic eye detector for open eyes if they might wear glasses.
...
void initWebcam(VideoCapture &videoCapture, Input_t cameraNumber)
{
...
void recognizeAndTrainUsingWebcam(VideoCapture &videoCapture, CascadeClassifier &faceCascade, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2)
{
    ...
    while (true) {
        ...
        if (keypress == VK_ESCAPE) {   // Escape Key
            ...
            const auto current_frame = videoCapture.get(CAP_PROP_POS_FRAMES);
            videoCapture.set(CAP_PROP_POS_FRAMES, current_frame-20);
        }else if (keypress == 'd') {
            const auto current_frame = videoCapture.get(CAP_PROP_POS_FRAMES);
            videoCapture.set(CAP_PROP_POS_FRAMES, current_frame+20);
        }else if (keypress == 'r') {
            videoCapture.set(CAP_PROP_POS_FRAMES, 200);
        }
...
int main(int argc, char *argv[])
{
    ...
    // Get access to the webcam.
    initWebcam(videoCapture, "../data/meeting.mp4");
